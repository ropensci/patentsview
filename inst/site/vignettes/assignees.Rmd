...
The following is a brief analysis of the top organizations patenting in the field of databases. 
...
# Our query will look for "database" in either the patent title or abstract... This isn't a terribly good query, but it will work for the purpose of demonstration. Users who are interested in writing higher-recall queries could consult the large body of research that has been done re: patent document retrievable.
query <- with_qfuns(
  or(
...
top_asgns <-
  dl$assignees %>%
    filter(!is.na(assignee_organization)) %>% # some patents are not assigned to an org (only an inventor)
...
IBM is far and away the biggest player in the field. However, we can see that Oracle and Salesforce.com, Inc. are relatively more interested in this area, as indicated by the percentage of their patents that relate to databases.
...
4. Finally, let's see how these organizations compare across several metrics at once, including their average citation rate. First, we'll need to normalize the raw citation counts by publication year, so that older patents don't have an unfair advantage over younger patents (i.e., b/c they have had a longer time to accrue citations).
...
Then, we can create a scatter plot to visualize the data. Each assignee will be represented by a point/bubble. The x-value of the assignee will represent the total number of patents it has in the field of databases (on a log scale), while the y-value will represent the assignee's average normalized citation rate. The size of the bubble we be proportional to the fraction of the assignee's patents that relate to databases.
