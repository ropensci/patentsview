...
one_request <- function(method, query, base_url, arg_list, ...) {
  ...
  if (httr::http_error(resp)) throw_er(resp = resp)
...
#'
#' @param query Either a character/json vector of length 1 with valid JSON query syntax or a list to be converted to JSON. See examples and/or query vignette.
...
#' @param all_pages Do you want to download all possible pages of output? If \code{all_pages = TRUE}, the values of \code{page} and \code{per_page} don't matter.
#' @param sort A named character vector where the names indicate the field that is to be sorted by and the values indicate the direction of sorting ("asc" or "desc"). For example, \code{sort = c("patent_number" = "asc")} would be an acceptable value, as would \code{sort = c("patent_number" = "asc", "patent_date" = "desc")}, or \code{sort = NULL} (the default - no sorting is done).
