...
#'
#' @param query The query that the API will use to filter records. \code{query} can come in any one of the following forms:
#'  \itemize{
#'   \item A character or JSON element with valid JSON syntax\cr E.g., \code{'{"_gte":{"patent_date":"2007-01-04"}}'}
#'   \item A list which will be converted to JSON syntax by \code{search_pv} via \code{\link[jsonlite]{toJSON}}\cr E.g., \code{list("_gte" = list("patent_date" = "2007-01-04"))}
...
#' @param endpoint The web service resource you wish to search. \code{endpoint} must be one of the following: "patents", "inventors", "assignees", "locations", "cpc_subsections", "uspc_mainclasses", or "nber_subcategories".
...
#' @param sort A named character vector where the name indicates the field to sort by and the value indicates the direction of sorting (direction should be either "asc" or "desc"). For example, \code{sort = c("patent_number" = "asc")} or \cr\code{sort = c("patent_number" = "asc", "patent_date" = "desc")}. \code{sort = NULL} (the default) means do not sort the results. You must include any fields that you wish to sort by in \code{fields}.
#' @param method The HTTP method that you want to use to send the request. Possible values include "GET" or "POST". \strong{Use the POST method when your query is very long (say, over 2,000 characters in length)}.
#' @param error_browser The program used to view the HTML containing any error messages sent by the API. This should be one of the following:
#' \itemize{
#'   \item The string "false" (the default), which turns error browsing off. Instead of viewing the HTML in this case, you will get a portion of the parsed HTML text returned to you as an R error.
#'   \item The name of the program to use to view the HTML error message. If the name of the program is on your PATH, just specify the program name (e.g., \code{error_browser = "chrome"}). Otherwise, include the full path to the program.
#'   \item An R function to be called to invoke the browser (e.g., \code{error_browser = rstudioapi::viewer})
#'   \item Under Windows, a value of \code{NULL} is allowed and implies that the file association mechanism will be used to determine which browser is used.
#'  }
...
#' \describe{
#'   \item{data}{A list with one element - a named data frame containing the data returned by the server. Each row in the data frame corresponds to a single primary entity. For example, if you search the assignee endpoint, then the data frame will be on the assignee-level, where each row corresponds to a single assignee. Fields that are not on the assignee-level would be returned in a list column.}
#'   \item{query_results}{Entity counts across all pages of output (not just the page returned to you). If you set \code{subent_cnts = TRUE}, you will be returned both the counts of the primary entities and the subentities.}
