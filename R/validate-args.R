validate_args <- function(query, fields, endpoint, method, subent_cnts,
  ...
  asrt(all(endpoint %in% ok_ends, length(endpoint) == 1),
       paste0("endpoint must be a length 1 vector and be one of: ",
              paste(ok_ends, collapse = ", ")))
  asrt(all(method %in% ok_meth, length(method) == 1),
       paste0("method must be a length 1 vector and be one of: ",
  ...
  if (!is.null(sort))
    asrt(all(all(names(sort) %in% fields), all(sort %in% c("asc", "desc")),
             !is.list(sort)), "sort has to be a named character vector and each name has to be specified in the field argument. See examples")
