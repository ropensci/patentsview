...
This data frame has assignee-level columns that are vecters (e.g., `assignee_last_seen_date`) and subentity-level columns (applications subentity and government interest statement subentity) that are lists. We can then call `flatten_pv_data` to split the single data frame into three different data frames, one for each entity/subentity:
