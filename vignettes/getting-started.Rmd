...
## Converting the data returned by `search_pv()`

The API always returns the data fields as strings, even if they could be better represented using a different data type (e.g., numeric). You can cast all of the fields in the data frame returned by `search_pv()` into their preferred types `cast_pv_data()`:

```{r}
res <- search_pv(query = "{\"patent_number\":\"5116621\"}", 
                 fields = c("patent_date", "patent_title", "patent_year"))

# Right now all of the data fields are stored as characters:
res
