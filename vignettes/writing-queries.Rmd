...
Let's say you want to find all patents published in the last 10 years that have the word "dog" in their titles or abstracts and were assigned to either a US or Canadian organization. There are three different ways that you can create a query to pass to the `query` argument of `search_pv`:

1. Pass a raw character/JSON vector:
...
2. Pass a list (which will be converted to JSON by `search_pv`):
...
3. Pass an object of class `pv_query` (which is also a list). You create by using the domain specific language (DSL) provided in the `patentsview` package: 
...
Patents that have an inventor listed on them whose first name contains "joh" and has an abstract with either the phrase "dog bark" or "cat meow":
...
Patents with an inventor whose disambiguated last name is “Smith” and with “cotton gin” in the title, or with an inventor whose disambiguated last name is “Hopper” and with “COBOL” in the title (Adapted from [PatentsView example](http://www.patentsview.org/api/query-language.html)):
